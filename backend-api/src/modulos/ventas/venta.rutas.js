...
async function registrarRutasVenta(app) {
  app.get('/ventas', { preHandler: [app.requiereModulo('ventas')] }, async (req, res) => {
    const datos = await listarVentas()
    return res.send(datos)
  })

  app.get('/ventas/:id', { preHandler: [app.requiereModulo('ventas')] }, async (req, res) => {
  ...
  app.post('/ventas', { preHandler: [app.requierePermiso('VENDER'), app.requiereModulo('ventas')] }, async (req, res) => {
