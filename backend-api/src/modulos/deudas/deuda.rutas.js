...
async function registrarRutasDeuda(app) {
    ...
    app.get('/deudas', { preHandler: [app.autenticar] }, async (req) => {
    ...
    app.get('/deudas/cliente/:clienteId', { preHandler: [app.autenticar] }, async (req) => {
        const clienteId = Number(req.params.clienteId)
        return obtenerDeudasPorCliente(clienteId)
    })

    // Obtener una deuda específica
    app.get('/deudas/:id', { preHandler: [app.autenticar] }, async (req) => {
    ...
    app.post('/deudas/:id/abonos', { preHandler: [app.autenticar] }, async (req, res) => {
    ...
    app.get('/abonos/cliente/:clienteId', { preHandler: [app.autenticar] }, async (req) => {
        const clienteId = Number(req.params.clienteId)
        return obtenerAbonosPorCliente(clienteId)
    })

    // Marcar deudas vencidas (tarea programada o manual)
    app.post('/deudas/marcar-vencidas', { preHandler: [app.autenticar] }, async () => {
